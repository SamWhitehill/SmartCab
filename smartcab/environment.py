...
def unwrap_self_f(arg, **kwarg):
    return Environment.fnUpdateAgent(*arg, **kwarg)

def unwrap_self_fUpdateTraffic(arg, **kwarg):
    return Environment.fnUpdateTraffic(*arg, **kwarg)
...
class Environment(object):
    ...
    def step(self):
        ...
        #S.W.: 11/6/2016 changing for speed
        #changed below to list comp, hoping for speed improvement
        if True:
            myFn =self.fnUpdateAgent
            myIterable =self.agent_states.iterkeys()
            result =list(map(myFn,myIterable ))
            
            #for agent in self.agent_states.iterkeys():
            #    if agent is not self.primary_agent:
            #        agent.update()

        if False :#__name__=="environment":
            #lstPrimary=[self.primary_agent]
            #lstPrimary=lstPrimary*2 #len(self.agent_states)
            pool = multiprocessing.Pool(processes=3)
            
            
            result=pool.map(unwrap_self_f, zip([self]*len(self.agent_states), self.agent_states.iterkeys()))
            #pool.map(unwrap_self_f, zip([self]*len(names), names))
            pool.close()
            pool.join()

            #pool = multiprocessing.Pool(processes=3)
            #result=pool.map(self.fnUpdateAgent, zip(self.agent_states.iterkeys(),lstPrimary))
            
            #result=pool.map(unwrap_self_fUpdateTraffic, zip([self]*len(self.intersections),\
                                                            #self.intersections.iteritems()))
            #pool.map(unwrap_self_f, zip([self]*len(names), names))
            #pool.close()
            #pool.join()
        ...
        #S.W. speed up traffic light
        #result=[traffic_light.update(self.t) for intersection, traffic_light in self.intersections.iteritems()]
        myfn =self.fnUpdateTraffic
        myIterable =self.intersections.iteritems()
        result =list(map(myfn,myIterable))
    ...
    def fnUpdateAgent(self,args):
        #wrapper for updating agent in parallel
        pAgent =args
        if pAgent is not self.primary_agent:
            #if pAgent is not pPrimaryAgent:
            pAgent.update()
        
        return pAgent


    def fnUpdateTraffic(self, args):
        #ptraffic_light,intersection =args
        intersection, ptraffic_light=args

        ptraffic_light.update(self.t)
